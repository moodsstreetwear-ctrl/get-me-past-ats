...
(function () {
  ...
  const FILLER_TERMS = [
    ...
    "customer service" // useful proof, but usually not a blocker unless the whole job is customer-facing
  ];
  ...
  const FAMILY_RULES = [
    {
      ...
      proof: ["windows", "password", "ticket", "troubleshoot", "hardware", "printer", "workstation", "network", "technical support", "user support", "microsoft office"],
      jobs: ["Help Desk Technician", "Desktop Support Technician", "IT Support Specialist", "Computer Support Specialist", "Service Desk Analyst"]
    },
    {
      id: "manufacturing",
      title: "Manufacturing / Production",
      proof: ["machine", "production", "quality", "safety", "material", "forklift", "assembly", "warehouse", "pallet", "equipment"],
      jobs: ["Machine Operator", "Production Worker", "Manufacturing Associate", "Assembly Technician", "Material Handler"]
    },
    {
      id: "maintenance",
      title: "Industrial Maintenance",
      proof: ["maintenance", "plc", "electrical", "mechanical", "hydraulic", "pneumatic", "schematic", "vfd", "preventive maintenance", "cmms"],
      jobs: ["Maintenance Technician", "Industrial Maintenance Technician", "Maintenance Helper", "Facilities Technician", "Manufacturing Technician"]
    },
    {
      id: "electrical",
      title: "Electrical Helper / Apprentice",
      proof: ["electrical", "wiring", "outlet", "fixture", "conduit", "panel", "breaker", "hand tools", "power tools"],
      jobs: ["Electrical Helper", "Apprentice Electrician", "Low Voltage Technician", "Residential Electrical Helper", "Maintenance Helper"]
    },
    {
      id: "driving",
      title: "Driver / Delivery / CDL",
      proof: ["cdl", "driver", "delivery", "route", "dot", "pre-trip", "post-trip", "twic", "mvr", "safe driving"],
      jobs: ["Delivery Driver", "Route Driver", "CDL Trainee", "Warehouse Driver", "Driver Helper"]
    },
    {
      id: "retail_service",
      title: "Retail / Customer Service",
      proof: ["customer", "cashier", "pos", "sales", "returns", "inventory", "stock", "front desk", "phone", "service"],
      jobs: ["Customer Service Representative", "Retail Associate", "Front Desk Associate", "Call Center Representative", "Sales Associate"]
    },
    {
      id: "admin_office",
      title: "Office / Administrative Support",
      proof: ["office", "data entry", "records", "scheduling", "email", "excel", "microsoft", "documents", "filing", "administrative"],
      jobs: ["Office Assistant", "Administrative Assistant", "Data Entry Clerk", "Records Clerk", "Receptionist"]
    },
    {
      id: "property_management",
      title: "Property Management",
      proof: ["property", "tenant", "lease", "vendor", "budget", "work order", "yardi", "inspection", "contractor", "maintenance coordination"],
      jobs: ["Assistant Property Manager", "Property Manager", "Leasing Coordinator", "Facilities Coordinator", "Tenant Services Coordinator"]
    },
    {
      id: "healthcare_support",
      title: "Healthcare Support",
      proof: ["patient", "medical", "clinical", "cna", "vitals", "records", "appointment", "hipaa", "care", "clinic"],
      jobs: ["Medical Office Assistant", "Patient Service Representative", "CNA", "Caregiver", "Medical Records Clerk"]
    }
  ];
  ...
  function simplifyScore() {
    ...
    if (score >= 85) {
      message.innerHTML = `Strong match for <strong>${escapeHTML(job)}</strong>. The resume shows most core proof this job asks for.`;
    } else if (score >= 65) {
      message.innerHTML = `Fair match for <strong>${escapeHTML(job)}</strong>. Fix the biggest blockers before applying.`;
    } else {
      message.innerHTML = `Weak match for <strong>${escapeHTML(job)}</strong>. This resume is missing core hiring proof for this role.`;
    }
  }
  ...
  function simplifyApplyDecision() {
    ...
    let label = "Apply Now";
    let tone = "good";
    let reason = "Your experience closely matches the primary requirements of this position.";
    if (saysNo || score < 55) {
      label = "Do Not Apply Yet";
      tone = "danger";
      reason = "This resume does not currently show enough core proof for this position.";
    } else if (saysCareful || score < 75) {
      label = "Apply After Fixes";
      tone = "caution";
      reason = "The resume has some useful proof, but important hiring evidence is still missing.";
    }
    box.innerHTML = `
  ...
  function buildBlockers() {
    ...
    const combined = unique([...missing, ...fromRedFlags]);
    return combined
  ...
  function simplifyMissingProof() {
    ...
    const heading = container.closest("article")?.querySelector("h3");
    const eyebrow = container.closest("article")?.querySelector(".eyebrow");
    if (eyebrow) eyebrow.textContent = "Missing Proof";
    if (heading) heading.textContent = "Biggest Interview Blockers";
    if (!blockers.length) {
      container.innerHTML = score >= 75
        ? `<p class="empty-proof">✅ None detected. Your resume demonstrates the core requirements for this position.</p>`
        : `<p class="empty-proof">No clean blocker list was detected. Try a more detailed job post for a sharper scan.</p>`;
      return;
    }
    ...
    container.innerHTML = `
      <div class="blocker-list">
        ${critical.length ? `<p class="blocker-label critical">Critical</p>${critical.map(item => `<span class="chip blocker critical">${escapeHTML(item)}</span>`).join("")}` : ""}
        ${important.length ? `<p class="blocker-label important">Important</p>${important.map(item => `<span class="chip blocker important">${escapeHTML(item)}</span>`).join("")}` : ""}
      </div>
    `;
  }
  ...
  function simplifyProofFound() {
    ...
    const items = extractChipText(container)
      .filter(item => !isFiller(item))
      .sort((a, b) => importanceScore(b) - importanceScore(a))
      .slice(0, 12);
    if (!items.length) {
      if (card) card.classList.add("gmpt-hidden");
      return;
    }
  ...
  function simplifyDirection() {
    ...
    if (score >= 80) {
      box.innerHTML = `<div class="simple-result-card"><h4>Stay focused on this job type.</h4><p>This resume already has enough proof to target roles like this one. Use the generator next when it is ready.</p></div>`;
      return;
    }
    if (!matches.length) {
      box.innerHTML = `<div class="simple-result-card"><h4>Try a closer job post.</h4><p>This resume does not give enough clear proof to recommend better matches yet.</p></div>`;
      return;
    }
    box.innerHTML = `
      <div class="better-match-list">
        ${matches.map(match => `
          <div class="better-match-card">
            <strong>${escapeHTML(match.job)} <span>${match.score}%</span></strong>
  ...
  function polishReport() {
    simplifyScore();
    simplifyApplyDecision();
    simplifyJobType();
    simplifyMissingProof();
    simplifyProofFound();
    simplifyDirection();
    simplifyGenerator();
  }
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    if (results) {
      new MutationObserver(() => setTimeout(polishReport, 35)).observe(results, { childList: true, subtree: true, characterData: true });
    }
  });
})();
